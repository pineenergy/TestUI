...
void setup()
{
  ...
  else
  {
    ...
    // 타이머 생성
    // TimerHandle_t timer = xTimerCreate("1초 타이머", pdMS_TO_TICKS(100), pdTRUE, (void *)0, mainAnimation);

    // 타이머가 성공적으로 생성되었는지 확인
    // if (timer == NULL)
    // {
    //   Serial.println("타이머 생성에 실패하였습니다.");
    //   while (1)
    //     ;
    // }
